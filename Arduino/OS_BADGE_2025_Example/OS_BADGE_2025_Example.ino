...
#include <epd_images.h>             // header file with "JED!!" 128x96 EPD image array, and a custom 128x96 OS224 EPD image array
...
#define EPD_SIZE        EPD_1_44         //our EPD is the 2 inch 128x96 pixel monochrome display by Pervasive 
...
void setup() {
  ...
  //Setup EPD pins
  pinMode(EPD_BUSY, INPUT);            // set up EPD Busy pin as input
...
void loop() {
  ...
  //Change out the EPD image every COUNTDOWN_TIME ticks of epd_timer using 2 different images in epd_images.h, re-initialize the TFT object after EPD image push or else TFT SPI bus will stop working
  ...
  else if (epd_timer == ((2 * COUNTDOWN_TIME)-1)){tft.fillScreen(ST7735_RED);EPD.begin();EPD.image_0(Yin_Yang_bits); EPD.end();tft.initR(INITR_144GREENTAB);tft.setRotation(3);}                      // push image, re-init TFT                   
  else if (epd_timer == ((3 * COUNTDOWN_TIME)-1)){tft.fillScreen(ST7735_RED);EPD.begin();EPD.image_0(Horizon_bits); EPD.end();tft.initR(INITR_144GREENTAB);tft.setRotation(3);}                      // push image, re-init TFT                   
  else if (epd_timer == ((4 * COUNTDOWN_TIME)-1)){tft.fillScreen(ST7735_RED);EPD.begin();EPD.image_0(little_girl_bits); EPD.end();tft.initR(INITR_144GREENTAB);tft.setRotation(3);}                      // push image, re-init TFT                   
  else if (epd_timer == ((5 * COUNTDOWN_TIME)-1)){tft.fillScreen(ST7735_RED);EPD.begin();EPD.image_0(dog_bits); EPD.end();tft.initR(INITR_144GREENTAB);tft.setRotation(3);}                      // push image, re-init TFT                   
  else if (epd_timer == ((6 * COUNTDOWN_TIME)-1)){tft.fillScreen(ST7735_RED);EPD.begin();EPD.image_0(cisco_bits); EPD.end();tft.initR(INITR_144GREENTAB);tft.setRotation(3);}                      // push image, re-init TFT                   
  else if (epd_timer == ((7 * COUNTDOWN_TIME)-1)){tft.fillScreen(ST7735_RED);EPD.begin();EPD.image_0(wifi_bits); EPD.end();tft.initR(INITR_144GREENTAB);tft.setRotation(3);}                      // push image, re-init TFT                   
  else if (epd_timer == ((8 * COUNTDOWN_TIME)-1)){tft.fillScreen(ST7735_RED);EPD.begin();EPD.image_0(Guitar_bits); epd_timer = -1; EPD.end();tft.initR(INITR_144GREENTAB);tft.setRotation(3);}     // push image, re-init TFT, reset epd_timer
